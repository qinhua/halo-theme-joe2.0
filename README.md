...
2. 进入博客后台管理 `外观-主题-安装-远程下载`，贴入仓库地址进行安装（建议直接使用最新版本 ）。
   如果在线安装失败，可以选择 `本地上传`，到主题 [Release](https://github.com/qinhua/halo-theme-joe2.0/releases) 页面下载指定版本主题包，并上传上去；

3. 等待提示安装完成即可。

4. 更新主题建议通过 `外观-主题-Joe2.0-更多-从主题包更新` 把下载好的 `zip` 包上传上去，然后先切换到其他主题，再切回本主题，并进入主题设置执行一次保存，最后强刷前端页面即可（这里切主题主要是为了避免有时候主题状态未激活和缓存的问题）。

### ⚠️ 注意：

安装主题后请务必到 `后台管理 - 博客设置 - 高级选项` 中开启 `API服务` 并配置 `Access key` 为 `joe2.0`（切记要和主题设置中的 `AccessKey` 一致），不然部分用到 `Content API` 的页面会请求失败并报错。（请参考以下报错）
...
- 有时主题已经更新成功了，但是页面还是加载的旧版本的文件，可能是由于 `主题激活状态不正常` 导致的。此时只需要先启用其他主题再启用本主题即可。或者可以试着重启下后台试试。
...
目前主题中的留言页面是自定义页面，所以需要你进入 `后台管理-页面` ，点击 `新建页面` 添加留言页，然后把下面的模板复制进去并保存（建议直接复制 `leaving.ftl` 文件中最新的内容），最后进入 `外观-菜单` 添加刚才自定义的留言页，保存后刷新前台页面即可，其他自定义页面同理。
...
#### 8、自己修改了主题中的 CSS 或 JS 后为什么还是用的以前的？
